 ...
 * @param {String} [config.indexSuffixBy="halfYear"] elasticsearch index suffix for running index, one of m M <Monthly> q Q <Quarterly> h H <Bi-annually>
...
exports.requestHandler = function (config = {}, client) {
  ...
  const cfg = deepMerge(
    config,
    {
      node: config.host || "http://localhost:9200", // elastic new client use node
